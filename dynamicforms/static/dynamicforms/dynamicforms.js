...
dynamicforms = {
  ...
  refreshList: function refreshList(url, recordID, refreshType, formID, deletion, params) {
    if (refreshType == undefined || refreshType == 'record') {
      ...
      dynamicforms.ajaxWithProgress({
        ajax_setts: {
          type: 'GET', url: url, data: data, dataType: 'html',
  ...
  insertRow: function insertRow($newRow, formID, prevId) {
    ...
    if ($newRow.length) {
      ...
      if (hasPrevId) {
        ...
        $lastRow = $('#list-' + formID + ' > tbody > tr[data-id]').last();
  ...
  refreshRow: function refreshRow(data, formID, recordID) {
    ...
    if (recordID) {
      ...
      var $editedRow = $htmlObject.find('table[id^=\'list-\'] > tbody > ' + trSelector); // Edited record from ajax
      // returned html
  ...
  fieldSetVisible: function fieldSetVisible(field, visible) {
    //TODO: we need to check parent container if everything inside it is hidden. If there is, the parent container
    // needs to hide too
    var $field = field instanceof jQuery ? field : dynamicforms.field_helpers.get(field, '$field');
  ...
  * Callback function to redirect to login page
  ...
  defaultFilter: function defaultFilter(event) {
    ...
    var formId = (dynamicforms.DYNAMICFORMS.jquery_ui) ?
      $(event.currentTarget).parents('div.accordion').find('div.ui-accordion-content')
        .find('table[id^="list-"]')[0].getAttribute('id').replace('list-', '') :
      $(event.currentTarget).parents('div.card').find('div.card-body').find('table[id^="list-"]')[0].getAttribute('id')
        .replace('list-', '');
    dynamicforms.filterData(formId);
  },
  ...
  orderingColumnClicked: function orderingColumnClicked(event, $col, formID) {
    ...
    if (event.altKey) {
      ...
      if (ordering_index.direction === null) {
        ordering_index.direction = true;
      } else if (ordering_index.direction === true) {
        ordering_index.direction = false;
      } else {
  ...
  renderOrderingIndicator: function renderOrderingIndicator($col, ordering_index) {
    ...
    if ($col.hasClass('ordering')) {
      if (ordering_index.direction === true) {
        dir = '\u2191';
      }// ascending
  ...
  setCurrentOrder: function setCurrentOrder(formID, ordering) {
    ...
    $('#list-' + formID + '>thead>tr>th').each(function () {
      ...
      if ($col.hasClass('ordering')) {
        ...
        if (ordr == null || ordr.index < 1) {
          $col.addClass('unsorted');
        } else {
