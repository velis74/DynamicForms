...
dynamicforms = {
    ...
    showProgressDlg: function showProgressDialog(progressDlgID, progressSettings) {
        ...
        if (dynamicforms.DYNAMICFORMS.jquery_ui) {
            ...
            if (dynamicforms.shouldShowProgressDlg) {
                ...
                progressDlg.dialog({
                    dialogClass: 'progress-bar', closeOnEscape: false, resizable: false,
                    open: function () {
                        dynamicforms.progressDlgShown = true;
    ...
    refreshList: function refreshList(url, recordID, refreshType, formID, deletion, params) {
        if (refreshType == undefined || refreshType == 'record') {
            ...
            if (recordID) {
                ...
                $rowToRefresh = $('#list-' + formID).find(trSelector); // Row to refresh
            }
    ...
    insertRow: function insertRow(formId, $newRow) {
        ...
        // Insert new row after the last row or insert first row
        if ($newRow.length) {
            var $lastRow = $tableSelector.find("tr[data-id]").last(); // Last row before adding new record
            if ($lastRow.length) {
    ...
    refreshRow: function refreshRow(data, formID, recordID) {
        ...
        if (recordID) {
            var trSelector = "tr[data-id='" + recordID + "']";
            $rowToRefresh = $(tableSelector).find(trSelector); // Row to refresh
        }
            ...
            if (recordID) {
                ...
                if ($editedRow.length) {
                    if ($rowToRefresh != null && $rowToRefresh.length)
                        ...
                        dynamicforms.insertRow(formID, $editedRow);
                }
            } else {
                var $newRow = $($htmlObject.find('table')[tableIndex]).find("tr[data-id]").last(); // Added record from ajax returned html
     ...
     *
     * @param listId: table html element identifier
     * @param recordID: data-id attribute of table row
     */
    removeRow: function removeRow(listId, recordID) {
        var $trToRemove = $('#list-' + listId).find("tr[data-id='" + recordID + "']");
        $trToRemove.first().remove();
    ...
    wasLastRowDeleted: function wasLastRowDeleted(listId) {
        var $tableSelector = $('#list-' + listId);
        var $leftTrsCount = $tableSelector.find("tr[data-id]").length;
        if ($leftTrsCount == 0) {
            // Count how many lines should "No data" element span
            var colCount = $tableSelector.find("th").length;
            var noDataElement = "<tr data-title='NoData'><td colspan=" + colCount + " style='text-align: center'>No data</td></tr>";
            $tableSelector.find("tbody").first().append(noDataElement);
        }
    },
    ...
    deleteRow: function deleteRow(recordURL, recordID, refreshType, listId) {
        ...
        dynamicforms.ajaxWithProgress({
            ...
            .done(function (dialogHTML) {
                ...
                if (refreshType == undefined || refreshType == 'record') {
                    dynamicforms.removeRow(listId, recordID);
                } else if (refreshType == 'table') {
