    ...
    * Standard DRF router URL patterns apply:
    ...
    def filter_queryset_field(self, queryset, field, value):
        ...
        if isinstance(model_meta.get_field(field), (models.DateField, models.DateTimeField)):
            ...
            for date_time_fmt in [settings.DATETIME_FORMAT, '%Y-%m-%dT%H:%M:%S', hm_iso_format, settings.DATE_FORMAT,
                                  '%Y-%m-%d']:
                try:
            ...
            return queryset.filter(
                **{field + '__gte': date_time, field + '__lt': date_time + timedelta(**timedelta_value)})
        else:
