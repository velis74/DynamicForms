...
export default defineComponent({
  ...
  methods: {
     ...
     */
    async dispatchAction(action: any, extraData?: any) {
      ...
      console.log('actionDFName', actionDFName);

      function getHandlersWithPayload(self: any, actionName: any) {
      ...
      function emitEvent(self: any, emitData: any) {
        self.$emit(...emitData);
        let parent = self;
        while (parent != null) {
      ...
      async function asyncSome(arr: Array<any>, fun: Function) {
        for (const e of arr) {
