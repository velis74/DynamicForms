...
import RowTypesEnum from '../table/row-types';
...
export default defineComponent({
  ...
  props: {
     ...
     */
    displayComponent: { type: Object, required: true, validator(value: any) { return value in ComponentDisplay; } },
  },
  data() { return { orderingCounter: this.consumer.ordering.counter }; },
  computed: {
    renderComponent() {
  ...
  watch: {
    'consumer.ordering': {
      handler() {
        if (this.orderingCounter !== this.consumer.ordering.counter) {
  ...
  methods: {
    actionDelete(actionData: any, payload: any) {
      this.consumer.deleteRow(payload);
      return true;
    },
    actionValueChanged(actionData: any, payload: any) {
    ...
    async actionEdit(actionData: any, payload: any, extraData: any) {
    ...
    actionSort(action: any, payload: any, extraData: any) {
      console.log('Heyooh!');
