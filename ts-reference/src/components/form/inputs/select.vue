...
export default defineComponent({
  ...
  data() {
    return {
      ...
      // todo: this should be set globally, not per-component. and declared at top of html itself
      selenium: true as boolean,
    };
  },
  computed: {
    ...
    result: {
      get() {
        if (this.selected) {
          return this.multiple ? this.selected.map((i: any) => i.id) : this.selected.id;
        }
        return '';
      },
      set(value: any) {
        if (value != null) {
          if (this.multiple) {
            ...
            this.selected = this.options.filter((o: any) => val.includes(`${o.id}`));
          } else {
            this.selected = this.options.find((o: any) => String(o.id) === String(value));
          }
        } else {
          this.selected = null;
        }
  ...
  mounted() {
    ...
    if (this.selenium) {
      window[`setSelectValue ${this.field.uuid}`] = (value: any) => {
        this.result = value;
      };
    }
  },
